...
fn main() {
    ...
    // On MSVC, use static CRT (/MT) to match the rest of the Rust project.
    // Without this, the cmake crate defaults to /MD which causes LNK2038.
    #[cfg(target_env = "msvc")]
    {
        config.static_crt(true);
        // Also override CMake's per-config defaults which hardcode /MD.
        config.define("CMAKE_MSVC_RUNTIME_LIBRARY", "MultiThreaded");
        config.define("CMAKE_CXX_FLAGS_DEBUG", "/MTd /Zi /Ob0 /Od /RTC1");
        config.define("CMAKE_CXX_FLAGS_RELEASE", "/MT /O2 /Ob2 /DNDEBUG");
        config.define("CMAKE_CXX_FLAGS_MINSIZEREL", "/MT /O1 /Ob1 /DNDEBUG");
        config.define("CMAKE_CXX_FLAGS_RELWITHDEBINFO", "/MT /Zi /O2 /Ob1 /DNDEBUG");
        config.define("CMAKE_C_FLAGS_DEBUG", "/MTd /Zi /Ob0 /Od /RTC1");
        config.define("CMAKE_C_FLAGS_RELEASE", "/MT /O2 /Ob2 /DNDEBUG");
        config.define("CMAKE_C_FLAGS_MINSIZEREL", "/MT /O1 /Ob1 /DNDEBUG");
